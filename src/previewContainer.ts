    ...
    public handleResult(pythonResults: PythonResult) {
        ...
        try {
            ...
            if (error instanceof Error || error instanceof String) {
                vscode.window.showErrorMessage("Internal ConfigView Error: " + error.toString(), "Report bug").then((action) => {
                    if (action == "Report bug") {
                        const bugReportLink = vscode.Uri.parse(`https://github.com/Almenon/ConfigView-vscode/issues/new?template=bug_report.md&title=${error}`)
